...
class HistoriqueProvider with ChangeNotifier {
  ...
  Future<void> fetchAndSetHistorique() async {
  ...
  void addFavoris(String marker) {
    _historique.add(marker);

    notifyListeners();
  }

  /// @return la liste de favoris de l'utilisateur
  List<String> get historique {
    return [..._historique];
  }
  ...
  void delete(String id, CustomMarker marker) {
    _historique.remove(id);
    _markers.remove(marker);

    FirebaseFirestore.instance
        .collection("utilisateur")
        .doc(FirebaseAuth.instance.currentUser.uid)
        .update({"historique": _historique});
    notifyListeners();
  }
